...
/// WebSocketChannel adapter implementing IRealtimeClient interface
/// Follows Adapter Pattern - wraps web_socket_channel to match our interface
/// This allows easy mocking, testing, and swapping of WebSocket libraries
class WebSocketChannelAdapter implements IRealtimeClient {
...
class WebSocketClientFactory {
  static WebSocketChannelAdapter createMessagesClient(String? token) {
    final client = WebSocketChannelAdapter();
    client.setAccessToken(token);
    return client;
  }

  static WebSocketChannelAdapter createNotificationsClient(String? token) {
    final client = WebSocketChannelAdapter();
    client.setAccessToken(token);
    return client;
  }

  static WebSocketChannelAdapter createAlertsClient(String? token) {
    final client = WebSocketChannelAdapter();
    client.setAccessToken(token);
    return client;
  }

  static WebSocketChannelAdapter createPresenceClient(String? token) {
    final client = WebSocketChannelAdapter();
    client.setAccessToken(token);
    return client;
  }
